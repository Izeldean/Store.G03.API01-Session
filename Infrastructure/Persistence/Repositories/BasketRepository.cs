...
{
    public class BasketRepository(IConnectionMultiplexer connection) : IBasketRepsository
    {
        ...
        {
            var radisValue = JsonSerializer.Serialize(basket);
           var flag = await _database.StringSetAsync(basket.id, radisValue, TimeSpan.FromDays(30));
